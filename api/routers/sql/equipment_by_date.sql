...
-- @obekt -код объекта
-- @tip_oborud - коду типа оборудования - Этот параметр передаем прямо в запросе т.к. используется конструкция in ()
...
-- statusName - название состояния оборудования, например "в резерве"
...
-- declare  @tip_oborud integer - Этот параметр передаем прямо в запросе т.к. используется конструкция in ()

set @d1 = convert(datetime, '?date', 104)
set @obekt = ?objectID
-- set @tip_oborud = ?equipmentTypeID - Этот параметр передаем прямо в запросе т.к. используется конструкция in ()

select
obekt.nazvanie as objectName,
tip_oborud.nazvanie as equipmentName,
oborud.tech_num as equipmentID,
sos_oborud.nazvanie as statusName,
ISNULL(
...
from (select max(a_sost_oborud.vremya_izm_sost) as max_vremya, a_sost_oborud.oborud as oborud
      ...
      where (oborud.obekt = @obekt) and (oborud.tip_oborud in (?equipmentTypeID)) and (a_sost_oborud.vremya_izm_sost<= @d1)
...
join tip_oborud on oborud.tip_oborud= tip_oborud.cod
join obekt on oborud.obekt= obekt.cod
where (oborud.obekt = @obekt) and (oborud.tip_oborud in (?equipmentTypeID)) and (sos_oborud.cod<>17)
